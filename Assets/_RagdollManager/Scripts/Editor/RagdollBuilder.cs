...
namespace MLSpace
{
    public class RagdollBuilder : ScriptableWizard
    {
        ...
        void OnWizardCreate()
        {
            ...
            Undo.SetCurrentGroupName("Create RagdollManager");
        ...
        protected override bool DrawWizardGUI()
        {
            ...
            {
                Debug.LogError("object cannot be null. Aborting...");
                return false;
            }
            if (!currentAnimator)
            {
                currentAnimator = currentObject.GetComponent<Animator>();
                if (!currentAnimator) { Debug.LogError("Cannot find 'animator' component." + " < " + this.ToString() + ">"); return false; }
            }
            if (!currentAnimator.isHuman) { errorString = "Avatar animation type is not humanoid"; return false; }

            bool ok = base.DrawWizardGUI();

            

            bool createFromHum = GUILayout.Button("Create From Humanoid Bones");

            if (createFromHum)
            {

                pelvis = currentAnimator.GetBoneTransform(HumanBodyBones.Hips);
                leftHips = currentAnimator.GetBoneTransform(HumanBodyBones.LeftUpperLeg);
                leftKnee = currentAnimator.GetBoneTransform(HumanBodyBones.LeftLowerLeg);
                leftFoot = currentAnimator.GetBoneTransform(HumanBodyBones.LeftFoot);
                rightHips = currentAnimator.GetBoneTransform(HumanBodyBones.RightUpperLeg);
                rightKnee = currentAnimator.GetBoneTransform(HumanBodyBones.RightLowerLeg);
                rightFoot = currentAnimator.GetBoneTransform(HumanBodyBones.RightFoot);
                leftArm = currentAnimator.GetBoneTransform(HumanBodyBones.LeftUpperArm);
                leftElbow = currentAnimator.GetBoneTransform(HumanBodyBones.LeftLowerArm);
                rightArm = currentAnimator.GetBoneTransform(HumanBodyBones.RightUpperArm);
                rightElbow = currentAnimator.GetBoneTransform(HumanBodyBones.RightLowerArm);
                middleSpine = currentAnimator.GetBoneTransform(HumanBodyBones.Chest);
                head = currentAnimator.GetBoneTransform(HumanBodyBones.Head);
                isValid = true;
                errorString = "";
                PrepareBones();
                CheckConsistency();
